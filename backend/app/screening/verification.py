import re

from app.rag.retrieval import retrieve_and_rerank


ALIASES = {
    "python programming": "Python",
    "python programming skills": "Python",

    "sql databases": "SQL",
    "sql database": "SQL",
    "sql/database knowledge": "SQL",

    "experience working with git and github": "Git and GitHub",

    "good problem solving skills": "Problem solving",
}


SUBSTRING_OK = {}


def normalize_requirement(requirement: str) -> str:
    key = requirement.strip().lower()
    return ALIASES.get(key, requirement)


def normalize_text(text: str) -> str:
    text = text.lower()

    text = re.sub(
        r"[^a-z0-9+#.\- ]",
        " ",
        text
    )

    text = re.sub(
        r"\s+",
        " ",
        text
    )

    return text.strip()


def _term_matches(term: str, evidence: str) -> bool:

    if re.search(
        rf"\b{re.escape(term)}\b",
        evidence
    ):
        return True

    for allowed_word in SUBSTRING_OK.get(term, []):
        if allowed_word in evidence:
            return True

    return False


def exact_skill_match(
    skill: str,
    evidence_text: str
) -> bool:

    skill_terms = re.findall(
        r"[a-z0-9+#.-]+",
        skill.lower()
    )

    evidence_norm = normalize_text(
        evidence_text
    )

    return all(
        _term_matches(term, evidence_norm)
        for term in skill_terms
    )


def select_best_evidence(
    evidence,
    max_results=2
):

    if not evidence:
        return []

    sorted_evidence = sorted(
        evidence,
        key=lambda x: x.get(
            "rerank_score",
            float("-inf")
        ),
        reverse=True
    )

    return sorted_evidence[:max_results]


def check_skill(
    skill: str,
    candidate_id: str = None
) -> dict:

    original_skill = skill

    normalized_skill = normalize_requirement(
        skill
    )

    print(normalized_skill)

    evidence = retrieve_and_rerank(
        normalized_skill,
        candidate_id=candidate_id
    )

    if not evidence:

        return {
            "skill": original_skill,
            "normalized_skill": normalized_skill,
            "matched": False,
            "evidence": []
        }

    # Only inspect the strongest evidence chunks.
    evidence = select_best_evidence(
        evidence,
        max_results=3
    )

    verified_evidence = []

    for item in evidence:

        document = item["document"]

        if exact_skill_match(
            normalized_skill,
            document
        ):
            verified_evidence.append(item)

    # Nothing explicitly proves the requirement.
    if not verified_evidence:

        return {
            "skill": original_skill,
            "normalized_skill": normalized_skill,
            "matched": False,
            "evidence": []
        }

    verified_evidence = select_best_evidence(
        verified_evidence,
        max_results=2
    )

    return {
        "skill": original_skill,
        "normalized_skill": normalized_skill,
        "matched": True,
        "evidence": verified_evidence
    }